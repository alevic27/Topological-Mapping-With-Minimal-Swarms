...
class NavAviary(CtrlAviary):
    ...
    """Gli input aggiunti sono:
        - sensors_attributes : booleano, mettere True se ci sono i RangeFinders
        - max_sensors_range : gittata RangeFinders (TODO CONTROLLARE SIA IN METRI)
        - ref_distance : int = 0,
        - s_WF: int = -1,
        - c_omega : float = 0,
        - c_vel: float = 0,
    ...
    def __init__(self,
        ...
        if self.SENSOR_ATTR :
            ...
            # self.rot_mat = np.array(p.getMatrixFromQuaternion(self.quat[self.nth_drone, :])).reshape(3, 3)
            self.rot_mat = np.zeros((3,3))
            self.sensor_direction = np.dot(self.sensor_direction,self.rot_mat)   # (3, 3) * ()
    ################################################################################
        ...
        if self.SENSOR_ATTR:
            for i in range(self.NUM_DRONES):
                ...
                for j in range(self.NUM_SENSORS) :
                    ...
                    self.rot_mat = np.array(p.getMatrixFromQuaternion(self.quat[self.nth_drone, :])).reshape(3, 3)
                    self.sensor_direction = np.dot(self.sensor_direction,self.rot_mat)  # TODO verificare che non serva trasporre rot_mat
                    ...
                    if closest_hit[j][0] != -1:
                        hit_distance = closest_hit[j][2]*self.max_range
    ...
    def _WallFollowingandAllign(self):            ### B.2
        td = 0.02
        omega=[]
    ...
    def _WallFollowing(self,WFstate):                    ### OCCHIO A USARE STATE COME VARIABILE CHE è anche il vettore degli stati, use WFstate
        td = 0.02
        ...
        for j in range(self.NUM_DRONES):
            ...
            if WFstate == 0: #ruoto per allinearmi al muro                             #### non inizializzato
